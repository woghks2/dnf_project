...
jobs_code = [('0_1', '웨펀마스터'), ('0_2', '소울브링어'), ('0_3', '버서커'), ('0_4', '아수라'), ('0_5', '검귀'),
 ...
 ('16_1', '뮤즈'), ('16_2', '트래블러'), ('16_3', '헌터'), ('16_4', '비질란테'),
...
# %%% [2-3] set_fame
def set_fame(driver, max_fame: int, end_fame: int):

    # 1. 탐색 범위보다 낮은 명성일 경우 탈출
    if max_fame < end_fame:
        return False
...
def scrape_characters(driver, job_name: str, max_fame: int):
...
def save_to_csv(data: list, min_fame: int, save_path: str, file_name: str):
    
    # 1. 데이터 프레임 생성 이후 이스핀즈 미만 명성 절사
    df = pd.DataFrame(data)
    df = df[df['fame'] >= min_fame]
    df['job_name'] = [file_name]*len(df)
...
# %% [4] run
def run(start_fame: int, end_fame: int):
    
    # 1. base path에 데이터를 저장할 폴더 생성
    today = datetime.now().strftime("%Y%m%d")
    base_path = BASE_PATH
    folder_name = f"data_{today}"
    save_path = os.path.join(base_path,'data','crawling_data',folder_name)
    ...
    for job_id, job_name in jobs_code:
        ...
        while True:
            ...
            if not set_fame(driver, max_fame, end_fame):  
                break
            time.sleep(0.4) # HTML 업데이트 대기. 짧게 해도 duplicate로 중복 제거
            
            # 5. 데이터 업데이트
            data,min_fame = scrape_characters(driver, job_name, max_fame)
            datas.extend(data)

            # 정보가 없어서 업데이트 되지 않는 경우
            if max_fame == min_fame:
                max_fame -= 2000
            # 가장 낮은 명성을 최대 명성 범위로 업데이트
            else:
                max_fame = min_fame
        ...
        save_to_csv(datas, end_fame, save_path, job_name)
